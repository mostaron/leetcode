...
public class Solution621 {
    public int leastInterval(char[] tasks, int n) {
        HashMap<Character,Integer> countedTasks = new HashMap<>();
        for(int i=0; i<tasks.length; i++){
            if(countedTasks.containsKey(tasks[i])){
                countedTasks.put(tasks[i], countedTasks.get(tasks[i])+1);
            }else{
                countedTasks.put(tasks[i], 1);
            }
        }

        Map<Character, Integer> sortedTasks = countedTasks.entrySet().stream()
        ...
        while(!sortedTasks.isEmpty()){
            count += cooldown;
            cooldown = n+1;
            ArrayList<Character> removeKeys = new ArrayList<>();

            for(Character task : sortedTasks.keySet()){
                count++;
                System.out.print(task);
                if(sortedTasks.get(task) > 1){
                    sortedTasks.put(task, sortedTasks.get(task) -1);
                }else{
                    removeKeys.add(task);
                }
            ...
            for(Character task : removeKeys){
                sortedTasks.remove(task);
            }

            sortedTasks = sortedTasks.entrySet().stream()
                    .sorted(Map.Entry.<Character, Integer>comparingByValue().reversed())
                    .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue,
                            (e1, e2) -> e1, LinkedHashMap::new));
